...
public class GaugeSpeed extends View {
	...
	public static final int SCALE_DIVISIONS = 20;
	...
	public static final float[] RANGE_VALUES = {200.0f };
	...
	private void initDrawingTools() {
		...
		warningpaint =  new Paint(Paint.LINEAR_TEXT_FLAG | Paint.ANTI_ALIAS_FLAG);
		warningpaint.setColor(Color.YELLOW);
		warningpaint.setStyle(Paint.Style.FILL);
		warningpaint.setStrokeWidth(0.005f);
		warningpaint.setTextSize(0.045f);
		warningpaint.setTypeface(Typeface.SANS_SERIF);
		warningpaint.setTextAlign(Align.CENTER);
		warningpaint.setShadowLayer(0.005f, 0.002f, 0.002f, mTextShadowColor);
	}
	...
	private Paint getDefaultOuterBorderPaint() {
		final Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		paint.setStyle(Paint.Style.FILL);
		paint.setColor(Color.argb(245, 0, 25, 51));
		return paint;
	}
	...
	public void setDefaultRanges() {
		mRangeValues = new float[] { 200 };
		mRangeColors = new int[] {Color.rgb(250, 250, 250)};
	}
	...
	public void setDefaultScaleRangePaints() {
		...
		for (int i = 0; i < length; i++) {
			...
			mRangePaints[i].setStrokeWidth(0.004f);
			mRangePaints[i].setTextSize(0.04f);
	...
	private void drawText(final Canvas canvas) {
		...
		if (!TextUtils.isEmpty(mTextUnit)) {
			canvas.drawText(mTextUnit, startX , startY + 0.06f, mTextUnitPaint);
		}
	}
	...
	private void drawScale(final Canvas canvas) {
		...
		for (int i = 0; i < totalTicks; i++) {
			...
			final float y3 = y1 + 0.035f; // height of subdivision

			final float value = getValueForTick(i);
			final Paint paint = getRangePaint(value);
			if (0 == value % (mScaleEndValue/mDivisions)) {
				// Draw a division tick
				if(value==50 || value==70 || value==110)
				{
				canvas.drawLine(0.5f, y1, 0.5f, y3, warningpaint);
				canvas.drawText(valueString(value), 0.5f, y3 + 0.045f, warningpaint);
				}
				else
				{
